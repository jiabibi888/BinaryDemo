...
//        _openUDID = [[UIDevice currentDevice] uniqueIdentifier];//UDID
//    }
    ...
    if (_openUDID==nil) {
        ...
        _openUDID = [NSString stringWithFormat:
                ...
                @(arc4random() % 4294967295).unsignedLongLongValue];
    }
...
//
+ (NSString*) value {
    NSString *idfv = [[[UIDevice currentDevice] identifierForVendor] UUIDString];//IDFV
    return idfv;
//    NSDebugLog(@"%@",[TCT_OpenUDID valueWithError:nil]);
//    return [TCT_OpenUDID valueWithError:nil];
}
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if(guuid == nil)
    {
        ...
        guuid = (__bridge_transfer NSString *) CFUUIDCreateString(NULL, uuid);//CFUUID
        CFRelease(uuid);
    }
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [TCT_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [TCT_OpenUDID _getOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [TCT_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
