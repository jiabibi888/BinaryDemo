...
{
    return [tc_GTMBase64 stringByEncodingData:self];
}
...
- (NSData *)tc_AESEncryptWithPassphrase:(NSString *)pass keybits:(AESKeyBits)kbs
{
	...
	int nrounds = tc_rijndaelSetupEncrypt(rk, key, KEYBITS);
	...
	while (1) 
	{
		...
		tc_rijndaelEncrypt(rk, nrounds, plaintext, ciphertext);
...
- (NSData *)tc_AESDecryptWithPassphrase:(NSString *)pass keybits:(AESKeyBits)kbs
{
	...
	int nrounds = tc_rijndaelSetupDecrypt(rk, key, KEYBITS);
	unsigned char *srcBytes = (unsigned char *)[self bytes];
	int index = 0;
	while (index < [self length])
	{
		...
		tc_rijndaelDecrypt(rk, nrounds, ciphertext, plaintext);
